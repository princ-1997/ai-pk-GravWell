...
5. 点击 **PLAY** — 启动 10 轮迭代基准测试
   - 每轮：所有玩家并行调用 LLM → 在同一模拟中竞技 → 各自获取诊断反馈
   - 每轮改进提示包含**完整进化历史**（分数趋势、逐船数据、最佳代码 + 最新代码）
   - 分数折线图实时展示各模型的学习曲线
6. 用**轮次滑块**切换查看第 1 轮（初始表现）到第 10 轮（最终进化结果）
7. 点击 **PLAY** 播放选中轮次的动画回放

> **Per-seed 结果缓存**: 对同一模型 + 种子重跑时，进度条会显示 `[cache]` —— 直接复用上次的代码与分数，**0 token 消耗**。每个玩家的调用状态实时显示（`P1:gen... P2:done P3:cache`），方便对比哪个模型响应最快。
...
- [x] **多玩家基准测试**（ADD PLAYER 注册模型 → 10 轮迭代竞技 → 学习曲线对比）
- [x] 逐轮回放（轮次滑块 + 分数折线图）
...
基于油管博主Wes Roth [Gravwell GPT](https://www.youtube.com/watch?v=bFO0uAMPx1g) 的设计理念，一个通过太空物理模拟来测试 LLM 代码能力的基准测试。
